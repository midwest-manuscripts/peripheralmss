...
in order of appearance **(See also BiblioPhilly Spreadsheet Metadata
Guidelines for field definitions)**:
...
There are three options for values for the collation field, depending on
whether you give a diagram to Dot, make a model in the Collation
Modeler, or generate a formula. Before ingestion, all manuscripts will
have a collation formula. Collation Modeler is here:
...
For Bibliographic citations, use the **modified Chicago Style for
notes** (with Last Name, First Name for first author; hyphens instead of
en-dashes for page ranges; p. and pp. for page numbers), as in the
following examples:
...
2.  Keywords are listed in the file `keywords.csv` within this
    repository.
...
7.  **If you want to add a new term** email the group (Dot, Erin, Amey,
    Nick, Diane, Andrew, and Jen Grayburn) to explain the need and we’ll
    deliberate. Terms will need to be added to the Google doc and then
    further added to the XML that is ingested into the TEI records.
